...
surface.blit(output, (background.get_width()//2 - output.get_width()//2, background.get_height()//2 - output.get_height()//2))
...
while done == False:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT + 1:
            ...
            xspeedknife = 10 #set the speed of the knife
        ...
        if event.type == pygame.USEREVENT + 4:
            ...
            if int (timer2 - 250) >= 1750:
                timer2 = int (timer2 - 250)
            elif int (timer2 - 250) < 1750:
                timer2 = 1750
    ...
    try:
        ...
        pygame.draw.rect(surface, 'red', (width - 145, yposSOL - 20,100, 10),0,10) 
        #if the soldier is not hit at all, he has a full green healthbar
        if hitsSOL == 0:
            pygame.draw.rect(surface, 'green', (width - 145, yposSOL - 20,100, 10),0,10)  
        #if the soldier is hit once, he has half a health bar
        if hitsSOL == 1:
            pygame.draw.rect(surface, 'green', (width - 145, yposSOL - 20,50, 10),0,10)
        #if the soldier is dead, he has no green health bar
        if hitsSOL == 2:
            pygame.draw.rect(surface, 'green', (width - 145, yposSOL - 20,0, 10),0,10)
    ...
    for goblin in (goblins2):
        ...
        #blit the goblin run animation using the goblin's [8] index animation cycler and its x and y positions on the screen
            ...
            pygame.draw.rect(surface, 'red', (goblin[0] + 30, goblin[1] + 5,100, 10),0, 15)
            #if the goblin has been hit two times, make the health bar that is green non-existant
            if goblin[5] == 2:
                pygame.draw.rect(surface,'green', (goblin[0] + 30, goblin[1] + 5, 0, 10), 0, 15)
            #if the goblin has only been hit once, make the health bar half green
            elif goblin[5] == 1:
                (pygame.draw.rect(surface,'green', (goblin[0] + 30, goblin[1] + 5, 50, 10)), 0, 15)
            #if the goblin has not been hit yet, make its healthbar fully green
            elif goblin[5] <= 0:
                pygame.draw.rect(surface,'green', (goblin[0] + 30, goblin[1] + 5, 100, 10), 0 ,15)
    ...
    #if a knife kills you, blit the images of the death images and account for a stupid error that does not get fixed
    try:
        if deathbyknife == True:
